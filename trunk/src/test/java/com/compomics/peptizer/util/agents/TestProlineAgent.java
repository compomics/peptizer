...
public class TestProlineAgent extends TestCase {
    ...
    public TestProlineAgent() {
        ...
        iProlineAgent.setProperty(ProlineAgent.INTENSITY, "0.4");
    }
    ...
    public void testInspect() {
        ...
        Assert.assertEquals(-1, lPeptideIdentification.getAgentReport(2, iProlineAgent.getUniqueID()).getReport(AgentReport.RK_ARFF));
        Assert.assertEquals("y11", lPeptideIdentification.getAgentReport(2, iProlineAgent.getUniqueID()).getReport(AgentReport.RK_TABLEDATA));
        Assert.assertEquals(AgentVote.NEGATIVE_FOR_SELECTION, lPeptideIdentification.getAgentReport(2, iProlineAgent.getUniqueID()).getReport(AgentReport.RK_RESULT));
     ...
     Assert.assertEquals(-1, lPeptideIdentification.getAgentReport(2, iProlineAgent.getUniqueID()).getReport(AgentReport.RK_ARFF));
     Assert.assertEquals("b10 - y11", lPeptideIdentification.getAgentReport(2, iProlineAgent.getUniqueID()).getReport(AgentReport.RK_TABLEDATA));
     Assert.assertEquals(AgentVote.NEGATIVE_FOR_SELECTION, lPeptideIdentification.getAgentReport(2, iProlineAgent.getUniqueID()).getReport(AgentReport.RK_RESULT));
     Assert.assertEquals(-1, lPeptideIdentification.getAgentReport(3, iProlineAgent.getUniqueID()).getReport(AgentReport.RK_ARFF));
