...
public class SpectrumpanelMcp extends JPanel {
    ...
    static {
        try {
            if (iKnownMassDeltas == null) {
                ...
                InputStream is = SpectrumpanelMcp.class.getClassLoader().getResourceAsStream("SpectrumPanel.properties");
                if (is != null) {
                    temp.load(is);
                    is.close();
     ...
     */
    public SpectrumpanelMcp(MascotGenericFile aSpecFile) {
     ...
     */
    public SpectrumpanelMcp(MascotGenericFile aSpecFile, boolean aEnableInteraction) {
     ...
     *                           be caught and dealt with.
     */
    public SpectrumpanelMcp(MascotGenericFile aSpecFile, int aDrawStyle, boolean aEnableInteraction) {
     ...
     */
    public SpectrumpanelMcp(double[] aMZ, double[] aIntensity, double aPrecursorMZ, String aPrecursorCharge, String aFileName) {
     ...
     */
    public SpectrumpanelMcp(MascotGenericFile aSpecFile, int aDrawStyle, boolean aEnableInteraction, Color aSpectrumFilenameColor) {
     ...
     *                  in this step as well.
     */
    public void setSpectrumFile(MascotGenericFile aSpecFile) {
        this.processSpectrumFile(aSpecFile);
    }

    /**
     * This method sets all the annotations on this instance. Passing a 'null' value for
     * the Vector will result in simply removing all annotations.
     *
    ...
    public void setAnnotations(Vector aAnnotations) {
        if (aAnnotations != null) {
            this.iAnnotations = aAnnotations;
        } else {
    ...
    public void paint(Graphics g) {
        ...
        if (iMasses != null) {
            ...
            drawAxes(g, iMassMin, iMassMax, iIntMin, iIntMax);
            drawPeaks(g);
    ...
    public void rescale(double aMinMass, double aMaxMass, boolean aNotifyListeners) {
        ...
        iMassMin = Math.max(aMinMass - (delta / 10), 0.0);
        iMassMax = aMaxMass + (delta / 10);
     ...
     *                  will be copied.
     */
    private void processSpectrumFile(MascotGenericFile aSpecFile) {
     ...
     */
    private int[] drawAxes(Graphics g, double aXMin, double aXMax, double aYMin, double aYMax) {
        ...
        this.drawXTags(g, aXMin, aXMax, xaxis, tempPadding);
        int yTemp = yaxis;
        if (iAnnotations != null && iAnnotations.size() > 0) {
            yTemp -= 20;
        }
     ...
     */
    private void drawXTags(Graphics g, double aMin, double aMax, int aXAxisWidth, int aPadding) {
        ...
        int numberTimes = (int) (aXAxisWidth / tagWidthEstimate);
        ...
        String filename = "Filename: " + iSpecFilename;
    ...
    private void drawYTags(Graphics g, double aMin, double aMax, int aYAxisHeight, int aPadding) {
        ...
        int numberTimes = (int) (aYAxisHeight / tagHeightEstimate);
    ...
    private void annotate(SpectrumAnnotation aSA, Graphics g, HashMap aAlReadyAnnotated) {
        double mz = aSA.getMZ();
        double error = aSA.getErrorMargin();
